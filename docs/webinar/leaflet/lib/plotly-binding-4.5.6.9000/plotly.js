    ...
    if (allSets.length > 0) {
      ...
      for (var i = 0; i < allSets.length; i++) {
        (function() {
          ...
          grp.var("filter").on("change", function crosstalkFilterChange(e) {
            //var dim = traceManager.highlight.opacityDim;
            //var visible = traceManager.highlight.selected.visible;
            //traceManager.highlight.opacityDim = 0;
            //traceManager.highlight.visible = false;
            crosstalkSelectionChange(e);
            //traceManager.highlight.opacityDim = dim;
            //traceManager.highlight.visible = visible;
          });
  ...
  if (keys === null) {
    ...
    if (traces.length > 0) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        if (opacity !== this.gd._fullData[i].opacity) {
          continue;
        }
